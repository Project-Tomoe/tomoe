...
describe("CORS, Logger, Form Data, and Swagger Features", () => {
  ...
  describe("OpenAPI / Swagger Generation", () => {
    it("should serve docs HTML and swagger JSON and correctly extract Zod/TypeBox schemas", async () => {
      ...
      expect(doc.paths["/users"].post.requestBody.content["application/json"].schema.type).toBe("object");
      expect(doc.paths["/users"].post.requestBody.content["application/json"].schema.properties.name.type).toBe("string");
      expect(doc.paths["/users"].post.requestBody.content["application/json"].schema.properties.role.enum).toEqual(["admin", "user"]);
