...
type RelicContextEntry<R extends AnyRelic> = R extends ProvidingRelic<infer Name, infer T>
  ? [Name] extends [never]
    ? {}
    : { [K in Name]: T }
  : {}
...
export function unite<Relics extends AnyRelic[]>(...relics: Relics): RelicGroup<Relics> {
  return {
    _kind: "group",
    relics,
